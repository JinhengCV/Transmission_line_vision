...
sys.path.append(os.path.join(os.path.dirname(__file__), 'parameters'))  # 加入 parameters 目录

from functools import lru_cache

from parameters_hk2 import camera_intrinsic, r, t, optimization, depth_map, segments, reps_concat, nz_cols_per_row, nz_vals_per_row
...
def remove_outliers(vals, z=3.0):
    v = np.asarray(vals)
    if v.size == 0:
        return v
    m = np.mean(v)
...
def _cached_key(center_y, left_x, right_x):
    return (center_y, left_x, right_x)

def _median_in_row_range(y, L, R):
    cols = nz_cols_per_row[y]
    if cols.size == 0:
        return None
    i0 = np.searchsorted(cols, L, side='left')
    i1 = np.searchsorted(cols, R, side='right')
    if i1 <= i0:
        return None
    vals = nz_vals_per_row[y][i0:i1]
    vals = remove_outliers(vals)
    if vals.size == 0:
        return None
    return float(np.median(vals))

def find_depth_in_range(left_x, right_x, center_y, max_expand=32, step=2):
    h, w = depth_map.shape
    L = max(0, int(left_x))
    R = min(w - 1, int(right_x))
    cy = int(center_y)
    if L > R or cy < 0 or cy >= h:
        return 0.0, (-1, -1)

    med = _median_in_row_range(cy, L, R)
    if med is not None:
        cx = (L + R) // 2
        return med, (cx, cy)
    ...
    for dy in (-1, 1):
        y = cy + dy
        if 0 <= y < h:
            med = _median_in_row_range(y, L, R)
            if med is not None:
                cx = (L + R) // 2
                return med, (cx, y)

    expand = step
    while expand <= max_expand and (L - expand >= 0 or R + expand < w):
        L2 = max(0, L - expand)
        R2 = min(w - 1, R + expand)
        med = _median_in_row_range(cy, L2, R2)
        if med is not None:
            cx = (L2 + R2) // 2
            return med, (cx, cy)
        for dy in (-1, 1):
            y = cy + dy
            if 0 <= y < h:
                med = _median_in_row_range(y, L2, R2)
                if med is not None:
                    cx = (L2 + R2) // 2
                    return med, (cx, y)
        expand += step
...
def _draw(cv_img, linep2ds, _mins, vis_cenpoints, vis_depth, height):
    ...
    """
    global alert_times, alert_count
    ...
    for i, _min in enumerate(_mins):
        ...
        if _min < 21:
            ...
            os.system('aplay -D plughw:1,0 /Disk500G/LJH/YOLOv7/alarm.wav >/dev/null 2>&1 &')
            alert_times.append(time.perf_counter() - t0)
            alert_count += 1
...
def get_alert_stats():
    ...
    global alert_times, alert_count
    if alert_count > 0:
        avg_delay = sum(alert_times) / alert_count
    else:
        avg_delay = 0
    return alert_count, avg_delay
