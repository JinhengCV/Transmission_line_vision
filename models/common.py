...
# from models.linear_att import FocusedLinearAttention
...
class Conv_att(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.att = FocusedLinearAttention(c2)
...
class DepthWiseConv(nn.Module):
    def __init__(self, inc, kernel_size, stride=1):
        ...
        #     nn.BatchNorm2d(inc),
        # )
        self.conv = Conv(inc, inc, kernel_size, stride, padding, inc)
