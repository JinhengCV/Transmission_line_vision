...
class VariFocalLoss(nn.Module):
    ...
    def forward(self, pred, target, iou_scores=None):
        ...
        if iou_scores is not None and torch.any(iou_scores > 0):
            # 对于正样本，使用 iou_scores 作为目标权重
            target = torch.where(target > 0, iou_scores, target)
        focal_weight = self.alpha * target * (1 - pred_sigmoid).pow(self.gamma) + \
    ...
    def __init__(self, model, autobalance=False):
        ...
        if g > 0:
            ...
            if varifocal: # varifocal loss for classification branch
                BCEcls, BCEobj = FocalLoss(BCEcls, alpha=a, gamma=g), FocalLoss(BCEobj, gamma=g, alpha=a)
            else:
...
# class ComputeLossOTA:  ###提出方法改进的,负样本挖掘机制
#     def __init__(self, model, autobalance=False, varifocal=True, hard_negative_ratio=3):
#         super(ComputeLossOTA, self).__init__()
#         self.iou_return = True
#         self.hard_negative_ratio = hard_negative_ratio  # 新增参数：负样本与正样本的比例
...
#                 iou_score, iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, CIoU=True, return_iou=self.iou_return)
#                 lbox += (1.0 - iou).mean()  # iou loss
...
#             # Enhanced Negative Sample Mining
#             # Compute negative sample confidence
#             neg_conf = pi[..., 4].sigmoid().detach()  # Confidence for all predictions
#             neg_conf[b, a, gj, gi] = float('inf')  # Exclude positive samples

#             # Sort negative samples by confidence (ascending order)
#             num_neg = min(len(neg_conf), n * self.hard_negative_ratio)
#             neg_indices = torch.argsort(neg_conf.view(-1))[:num_neg] #对所有负样本的索引进行排序，并选取置信度最低的前num_neg个样本的索引
            
#             # Update objectness target for selected negatives
#             tobj.view(-1)[neg_indices] = 0.0  # Assign 0 to selected negatives
...
#     def __init__(self, model, autobalance=False, varifocal=True):
#         super(ComputeLossOTA, self).__init__()
#         self.iou_return = True
#         device = next(model.parameters()).device
#         h = model.hyp
...
#         # Define criteria
#         base_loss_cls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device))
#         base_loss_obj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['obj_pw']], device=device))

#         if varifocal:
#             self.BCEcls = VariFocalLoss(alpha=0.75, gamma=2.0)
#         else:
#             self.BCEcls = FocalLoss(base_loss_cls, gamma=1.5, alpha=0.25)

#         self.BCEobj = base_loss_obj

#         self.varifocal = varifocal
...
#                 # iou_score, iou = bbox_mpdiou(pbox.T, selected_tbox, x1y1x2y2=False, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2, grid=torch.stack([gj, gi]), return_iou=self.iou_return)  # iou(prediction, target)
#                 lbox += (1.0 - iou).mean()  # iou loss
